...
export async function POST(request: NextRequest) {
  ...
  const sysTokens = getSystemPromptTokens(SYSTEM_PROMPT);
    ...
    async start(streamCtrl) {
      ...
      try {
        ...
        if (useParallel) {
          ...
          const smartPromise = callModelNonStreaming(groq, smartModelId, trimmedMessages, SYSTEM_PROMPT, queryType);

          // Stream fast model to user immediately
          let fastContent = '';
          let fastSucceeded = false;
          try {
            const fastStream = await callModelStreaming(groq, fastModelId, trimmedMessages, SYSTEM_PROMPT, queryType, FAST_MODEL_TIMEOUT_MS);
          ...
          for (let i = 0; i < modelsToTry.length; i++) {
            ...
            try {
              const aiStream = await callModelStreaming(groq, modelId, trimmedMessages, SYSTEM_PROMPT, queryType, REQUEST_TIMEOUT_MS);
